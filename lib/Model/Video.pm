...
has cam1 => qq(ffmpeg  -i 'rtsp://192.168.1.81:554/stream1' -f mpegts -codec:v mpeg1video  -b:v 1300k   -bf 0  - 2>/dev/null |); #-s 1280x720 -crf  50 -an 
...
sub init {
  ...
  my $server = $self->app->server_engine; # в хуке before_server_start
...
# подписаться на поток камеры $feed
sub subws {
  my ($self, $ws, $feed) = @_;
